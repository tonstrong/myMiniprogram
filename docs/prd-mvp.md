...
- 大模型能力层必须支持 Provider 可切换，不与任何单一模型厂商写死绑定
...
### 7.5 可切换大模型 Provider 设计

#### 7.5.1 设计目标
- 不在业务代码中写死某一家模型厂商
- 对衣物识别、风格抽取、穿搭推荐三类能力统一封装
- 支持按任务类型、成本、时延、成功率动态切换 Provider
- 支持 fallback、重试、限流、降级和统一日志记录

#### 7.5.2 抽象层结构
```mermaid
flowchart LR
  S1[衣物识别服务] --> GW[LLM Gateway]
  S2[风格包抽取服务] --> GW
  S3[推荐规划服务] --> GW
  GW --> RT[路由与策略层]
  RT --> PA[Provider Adapter A]
  RT --> PB[Provider Adapter B]
  RT --> PC[Provider Adapter C]
```

#### 7.5.3 统一模型能力接口
系统内部统一定义三类模型能力：
1. `extract_clothing_attributes`
2. `extract_style_pack`
3. `generate_outfit_recommendations`

所有能力均通过统一的任务协议访问，而不是直接调用某一家厂商 SDK。

#### 7.5.4 统一调用协议示意
```json
{
  "taskType": "generate_outfit_recommendations",
  "provider": "auto",
  "modelTier": "standard",
  "input": {},
  "constraints": {},
...
字段说明：
- `taskType`：能力类型
- `provider`：`auto` 或指定内部 Provider 标识
- `modelTier`：`cheap` / `standard` / `premium`
- `input`：任务输入数据
- `constraints`：强约束条件
- `outputSchema`：要求模型遵循的结构化输出定义

#### 7.5.5 路由与 fallback 策略
- 默认由路由层按任务类型选择主 Provider
- 当出现超时、限流、结构化输出失败、关键字段缺失时自动切换备用 Provider
- 重试次数、超时时间、Provider 优先级由配置中心控制
- 对同一任务保留 provider 切换链路，便于定位问题

#### 7.5.6 配置驱动原则
Provider 的选择必须由配置驱动，而不是写死在业务逻辑中。
...
```json
{
  "tasks": {
    "extract_clothing_attributes": {
      "primaryProvider": "provider_a",
      "fallbackProviders": ["provider_b"],
      "tier": "standard"
    },
    "extract_style_pack": {
      "primaryProvider": "provider_b",
      "fallbackProviders": ["provider_a"],
      "tier": "standard"
    },
    "generate_outfit_recommendations": {
      "primaryProvider": "provider_a",
      "fallbackProviders": ["provider_c"],
      "tier": "premium"
    }
  }
}
...
#### 7.5.7 结构化输出与校验要求
- 所有模型调用必须使用结构化输出 Schema
- 若结果不符合 Schema，不直接入库，先走解析失败或 fallback 流程
- 模型输出只作为候选结果，最终仍需业务校验器确认
- 推荐结果中的 item_id 必须由服务端二次校验
...
- 模型 Provider 切换时不得改变输出 Schema 和业务约束
...
| provider | string | 本次识别使用的 Provider |
| modelName | string | 本次识别使用的模型标识 |
| modelTier | string | 模型档位 |
| retryCount | int | 模型调用重试次数 |
| source | string | 上传来源 |
...
| provider | string | 规则抽取使用的 Provider |
| modelName | string | 规则抽取使用的模型标识 |
| modelTier | string | 模型档位 |
| version | int | 版本号 |
...
| provider | string | 推荐规划使用的 Provider |
| modelName | string | 推荐规划使用的模型标识 |
| modelTier | string | 模型档位 |
| retryCount | int | 模型调用重试次数 |
...
- 模型 Provider 选择由服务端 LLM Gateway 与配置中心处理，小程序端不直接感知具体厂商
...
| 方法 | 路径 | 目的 | 关键入参 | 关键出参 |
|---|---|---|---|---|
| GET | `/api/tasks/{taskId}` | 获取异步任务状态 | taskId | status, progress, resultSummary, providerMeta |
